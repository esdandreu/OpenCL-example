...
//             Updated to C++ Wrapper API v1.2 by Tom Deakin and Simon
//             McIntosh-Smith, October 2012 Updated to C++ Wrapper v1.2.6 by Tom
//             Deakin, August 2013 Updated by Tom Deakin, November 2018
//
//------------------------------------------------------------------------------
...
TEST(vadd_chain, vadd_chain_test)) {
    ...
    try {
        ...
        vadd(cl::EnqueueArgs(queue, cl::NDRange(count)), d_a, d_b, d_c, d_d,
            count);

        vadd(cl::EnqueueArgs(queue, cl::NDRange(count)), d_d, d_e, d_f, d_g,
            count);
        ...
        float tmp;
        for (int i = 0; i < count; i++) {
            tmp = h_a[i] + h_b[i] + h_c[i] + h_e[i] + h_f[i]; // assign element
                                                              // i of a+b+c+e+f
                                                              // to tmp
